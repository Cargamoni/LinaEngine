...
void DefaultEditorLevel::Initialize()
{
	...
	camera.entity = m_ECS->create();
	auto& camFreeLook = m_ECS->emplace<FreeLookComponent>(camera.entity);
	auto& camTransform = m_ECS->emplace<TransformComponent>(camera.entity);
	auto& camCamera = m_ECS->emplace<CameraComponent>(camera.entity);
	...
	directionalLight.entity = m_ECS->create();
	auto& dirLight = m_ECS->emplace<DirectionalLightComponent>(directionalLight.entity);
	auto& dirLightTransform = m_ECS->emplace<TransformComponent>(directionalLight.entity);
	...
	dirLightT = &m_ECS->get<TransformComponent>(directionalLight.entity);
	//m_ECS->emplace<MeshRendererComponent>(directionalLight.entity, smallCubeRenderer);


	ECSEntity floor;
	floor.entity = m_ECS->create();
	...
	m_ECS->emplace<TransformComponent>(floor.entity, object1Transform);
	m_ECS->emplace<MeshRendererComponent>(floor.entity, mr);
	...
	for (int i = 0; i < cubeSize; i++)
	{
		...
		m_ECS->emplace<TransformComponent>(entity.entity, object1Transform);
		m_ECS->emplace<MeshRendererComponent>(entity.entity, object1Renderer);
	}
	...
	for (int i = 0; i < pLightSize; i++)
	{
		...
		auto& pLight1 = m_ECS->emplace<PointLightComponent>(entity.entity);
		...
		ECSEntity visuals;
		visuals.entity = m_ECS->create();
		object1Transform.transform.scale = 0.1f;
		m_ECS->emplace<TransformComponent>(visuals.entity, object1Transform);
		m_ECS->emplace<MeshRendererComponent>(visuals.entity, smallCubeRenderer);
	}
	...
	for (int i = 0; i < sLightSize; i++)
	{
		...
		auto& sLight1 = m_ECS->emplace<SpotLightComponent>(entity.entity);
	...
	//quad.entity = m_ECS->create();
	//m_ECS->emplace<TransformComponent>(quad.entity, object1Transform);
	//MeshRendererComponent quadR;
	//quadR.material = quadMaterial;
	//quadR.mesh = &m_RenderEngine->GetPrimitive(Primitives::PLANE);
	//m_ECS->emplace<MeshRendererComponent>(quad.entity, quadR);
	//
	//object1Transform.transform.location = (Vector3(-2.5f, 0, 11.0f));
	//quad2.entity = m_ECS->create();
	//m_ECS->emplace<TransformComponent>(quad2.entity, object1Transform);
	//m_ECS->emplace<MeshRendererComponent>(quad2.entity, quadR);
	//
	//object1Transform.transform.location = (Vector3(-2.5f, 0, 7.5f));
	//quad3.entity = m_ECS->create();
	//m_ECS->emplace<TransformComponent>(quad3.entity, object1Transform);
	//m_ECS->emplace<MeshRendererComponent>(quad3.entity, quadR);
	//
	//object1Transform.transform.location = (Vector3(-2.1f, 0, 7.0f));
	//quad4.entity = m_ECS->create();
	//m_ECS->emplace<TransformComponent>(quad4.entity, object1Transform);
	//m_ECS->emplace<MeshRendererComponent>(quad4.entity, quadR);
	//
